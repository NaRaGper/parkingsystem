...
public class FareCalculatorService {
	...
	public static boolean hasBeenThere(Ticket ticket) {
		...
		try {
			...
			ResultSet resultSet = statement.executeQuery("SELECT COUNT(*) FROM ticket WHERE vehicle_reg_number='"+ ticket.getVehicleRegNumber() +"'");
	...
	public void calculateFare(Ticket ticket){
		...
		if (duration <= 30 * (1000*60)) { //If the time parked is less than 30 minutes, then we set the ticket price to 0
			ticket.setPrice(0);
			...
			if (hasBeenThere(ticket) == true) ticketPrice *= 0.95; //We apply the 5% discount
